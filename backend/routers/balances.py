...
def get_balances(
    ...
    # Prepare target currency map
    target_currency_map = {}
    for group_id in user_group_ids:
        group = groups_map.get(group_id)
        if group:
            target_currency_map[group_id] = group.default_currency or "USD"

    # Batch calculate balances for all groups at once
    all_group_balances = calculate_net_balances_batch(db, list(user_group_ids), target_currency_map)

    # Process results
    for group_id, net_balances in all_group_balances.items():
        group = groups_map.get(group_id)
        if not group:
            continue
            
        group_default_currency = target_currency_map[group_id]
