 ...
 */
const WhoPaidSheet: React.FC<WhoPaidSheetProps> = ({
    open,
    onClose,
    participants,
    payerId,
    ...
    // Re-seed each time it opens: the board polls, so a stale draft here would
    // quietly overwrite a handle typed on another device.
    useEffect(() => {
        if (!open) return;
        setSelected(payerId);
        setError(null);
        const current = participants.find((p) => p.id === payerId);
        setHandle(current?.venmo_username ?? '');
    }, [open, payerId, participants]);
    ...
    return (
        <Sheet open={open} onClose={onClose} label="Who paid" title="Who paid?">
            <div className="flex flex-col gap-4">
                <p className="text-[12.5px] text-sw-muted">
                    Whoever fronted the bill is who everyone else owes. It does not
                    have to be you, and they do not need a Splitwiser account.
                </p>
                ...
                {needsHandle && seat && (
                    <Field
                        ...
                        hint={
                            <>
                                Optional. {seat.display_name} has no Splitwiser
                                account, so the claim page has no other way to send
                                people to them. Anyone holding this tab&rsquo;s link
                                will see it.
                            </>
                        }
                    />
                ...
                {seat && seat.user_id !== null && (
                    <p className="text-[11.5px] text-sw-dim">
                        ...
                        handle comes from{' '}
                        {seat.id === meId ? 'your' : 'their'} account settings.
                    </p>
                )}
